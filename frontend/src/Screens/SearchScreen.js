...
export default function SearchScreen() {
  ...
  const sp = new URLSearchParams(search); // /search?category=simulator
  ...
  const [{ loading, error, games, pages, countGames }, dispatch] =
    useReducer(reducer, {
      loading: true,
      error: '',
    });
    ...
    const fetchData = async () => {
      try {
        const { data } = await axios.get(
          `/api/games/search?page=${page}&query=${query}&category=${category}&price=${price}&rating=${rating}&order=${order}`
        );
          ...
          {loading ? (
            ...
            <>
              <Row className="justify-content-between mb-3">
                <Col md={6}>
                  <div>
                    {countGames === 0 ? 'No' : countGames} Results
                    {query !== 'all' && ' : ' + query}
              ...
              <Row>
                {games.map((game) => (
                  <Col sm={6} lg={4} className="mb-3" key={game._id}>
